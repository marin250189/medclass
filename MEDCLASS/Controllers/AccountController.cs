...
namespace MEDCLASS.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            bool error = false;
            string userCompleteName = string.Empty;
            UserBusiness _userBusiness = new UserBusiness();
            try
            {
                Domain.User user = _userBusiness.GetById(model.UserName);
                if (user != null)
                {
                    bool match = model.Password == user.password;
                    if (match == false)
                    {
                    ...
                    else
                    {
                        ...
                        FormsAuthentication.SetAuthCookie(userCompleteName,true);
                        ApplicationUser userio = new ApplicationUser 
                        {
                            UserName = userCompleteName,
                            PasswordHash = model.Password,
                            Id = model.UserName
                        };
                        await SignInAsync(userio, model.RememberMe);
